  ...
  const writeCollection = (value, members) => {
    Object.keys(value).forEach((key) => {
      ...
      if (!Array.isArray(subvalue)) {
        ...
        let fixArrayOfArrayValue = false;
        subvalue.forEach((value, i) => {
          if (fixArrayOfArrayValue) {
            return;
          }
          // we need to re-evaluate the alternates every time
          const tag = getTag(subsyntax, key, value);

          if (tag === tags.resolution || tag === tags.rangeOfInteger) {
            if (!Array.isArray(value)) {
              value = subvalue;
              fixArrayOfArrayValue = true;
            }
          }
          ...
          writeValue(tag, value, subsyntax.members);
        });
      }
